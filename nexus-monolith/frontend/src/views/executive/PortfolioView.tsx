    ...
    if (loading) {
        return (
            <div className="space-y-6">
                ...
                <div className="h-[400px] bg-white rounded-xl border border-slate-200/60 animate-pulse"></div>
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] text-center space-y-4">
                ...
                <div>
                    <h3 className="text-lg font-semibold text-slate-800">Falha ao carregar portfólio</h3>
    ...
    return (
        <div className="space-y-6">
                ...
                <div>
                    <div className="flex items-center gap-2.5 mb-1">
                        ...
                        <div className="flex items-center gap-1.5 px-2.5 py-1 bg-purple-50 rounded-full">
                            ...
                            <span className="text-[11px] font-bold text-purple-600">{projects.length} projetos</span>
                        </div>
                    </div>
            ...
            <Card className="border-slate-200/60 overflow-hidden">
                <CardHeader className="border-b bg-gradient-to-r from-slate-50/80 to-transparent pb-4">
                    <CardTitle className="text-[13px] font-semibold text-slate-700">Projetos em Andamento</CardTitle>
                </CardHeader>
                <CardContent className="p-0">
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left">
                            <thead className="bg-slate-50/50 text-[11px] text-slate-400 font-semibold uppercase tracking-wider border-b border-slate-100">
                                <tr>
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {projects.map((project) => {
                                    ...
                                    return (
                                        <tr key={project.id} className="hover:bg-purple-50/30 transition-colors duration-200 group">
                                            <td className="px-5 py-3.5">
                                                <p className="font-semibold text-slate-800 text-[13px]">{project.name}</p>
                                            ...
                                            <td className="px-5 py-3.5 text-right">
                                                <p className="font-semibold text-slate-800 text-[13px]">{formatCurrency(project.budgetConsumed)}</p>
                                            ...
                                            <td className="px-5 py-3.5 text-center">
                                                <span className={clsx(
                                                    ...
                                                    project.criticalRisks > 0 ? "bg-rose-50 text-rose-600 ring-1 ring-rose-200" : "bg-slate-50 text-slate-400"
                                                )}>
                                            ...
                                            <td className="px-5 py-3.5">
                                                <div className="flex items-center gap-2">
                                                    <div className="w-6 h-6 rounded-full bg-gradient-to-br from-indigo-100 to-purple-100 flex items-center justify-center text-[9px] font-bold text-indigo-700">
                                                        {project.manager.split(' ').map(n => n[0]).join('')}
                                                    </div>
                                                    <span className="text-slate-600 text-[12px] font-medium">{project.manager}</span>
                                                </div>
                                            </td>
                                            <td className="px-5 py-3.5 text-right">
                                                <Button variant="ghost" size="sm" className="opacity-0 group-hover:opacity-100 text-purple-600 hover:text-purple-700 hover:bg-purple-50 rounded-lg text-[12px] transition-opacity duration-200">
                                                    Detalhes →
                                                </Button>
                                            </td>
                                        </tr>
