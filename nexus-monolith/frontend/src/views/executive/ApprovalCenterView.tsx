        ...
        return (
            <div className="space-y-6">
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {[1, 2].map(i => (
                        <div key={i} className="h-[220px] bg-white rounded-xl border border-slate-200/60 animate-pulse"></div>
                    ))}
                </div>
            </div>
        );
    ...
    if (error) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] text-center space-y-4">
                ...
                <div>
                    <h3 className="text-lg font-semibold text-slate-800">Falha ao carregar aprovações</h3>
            ...
            <div className="flex justify-between items-end">
                <div>
                    <div className="flex items-center gap-2.5 mb-1">
                        <h2 className="text-2xl font-bold tracking-tight text-slate-900">Central de Comando</h2>
                        {pendingCount > 0 && (
                            <div className="flex items-center gap-1.5 px-2.5 py-1 bg-amber-50 rounded-full">
                                <Inbox className="w-3 h-3 text-amber-600" />
                                <span className="text-[11px] font-bold text-amber-700">{pendingCount} pendente{pendingCount > 1 ? 's' : ''}</span>
                            </div>
                        )}
                    </div>
                            ...
                            className={clsx(
                                ...
                                isOverdue && !isResolved
                                    ...
                                    : isResolved
                                        ...
                                        : 'border-slate-200/60 hover:shadow-md hover:border-purple-200/60'
                            )}
                        >
                            {/* Colored top accent */}
                            <div className={clsx(
                            ...
                            <CardHeader className="pb-3 pt-4">
                                <div className="flex justify-between items-start">
                                    <span className={clsx(
                                        ...
                                        approval.requiredRole === 'C_LEVEL' ? 'bg-purple-50 text-purple-700 ring-1 ring-purple-200/50' : 'bg-blue-50 text-blue-700 ring-1 ring-blue-200/50'
                                    )}>
                                        {approval.requiredRole}
                                    </span>

                                    {isResolved ? (
                                        <span className={clsx("text-[11px] font-bold flex items-center gap-1", approval.status === 'APPROVED' ? 'text-emerald-600' : 'text-rose-500')}>
                                ...
                                <CardTitle className="text-[15px] mt-2 font-semibold">{approval.resourceType}</CardTitle>
                                <CardDescription className="text-[11px] font-mono text-slate-400">Ref: {approval.resourceId}</CardDescription>
                            </CardHeader>

                            <CardContent className="flex-1 flex flex-col justify-between pt-0">
                                <div className="mb-4">
                                    <p className="text-[13px] text-slate-600 line-clamp-2 leading-relaxed">{approval.description}</p>
                                ...
                                {!isResolved && (
                                    <div className="grid grid-cols-2 gap-2 mt-auto">
                                        <Button
                                            ...
                                            className="border-rose-200 text-rose-600 hover:bg-rose-50 hover:text-rose-700 rounded-lg text-[12px] h-9"
                                        >
