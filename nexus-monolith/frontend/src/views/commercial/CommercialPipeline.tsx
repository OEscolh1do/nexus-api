...
import { MoreHorizontal, Filter, LayoutDashboard } from 'lucide-react';
...
export const CommercialPipeline: React.FC = () => {
    ...
    return (
        <div className="h-full flex flex-col">
            <header className="flex items-center justify-between px-6 py-4 border-b border-slate-200/60 bg-white/80 backdrop-blur-sm">
                <div className="flex items-center gap-2.5">
                    ...
                    <div className="flex items-center gap-1.5 px-2.5 py-1 bg-blue-50 rounded-full">
                        ...
                        <span className="text-[11px] font-bold text-blue-600">{deals.length} deals</span>
                    </div>
                </div>
                <div className="flex gap-2">
                    <button className="text-[13px] text-slate-500 hover:text-slate-700 hover:bg-slate-50 px-3 py-1.5 rounded-lg transition-all duration-200 flex items-center gap-1.5">
            ...
            <div className="flex-1 overflow-x-auto p-6 bg-slate-50/50">
                <div className="flex gap-4 h-full min-w-max">
                    {STATUS_COLUMNS.map(status => {
                        ...
                        return (
                            <div key={status} className="w-80 flex flex-col bg-white/60 backdrop-blur-sm rounded-xl p-3 border border-slate-200/60">
                                {/* Column Header */}
                                <div className="flex items-center justify-between mb-3 px-1">
                                    <div className="flex items-center gap-2">
                                        <div className={clsx("w-2 h-2 rounded-full bg-gradient-to-br", STATUS_COLORS[status])}></div>
                                        <h3 className="font-semibold text-slate-700 text-[13px]">
                                ...
                                <div className="flex-1 space-y-2.5 overflow-y-auto pr-1">
                                    {columnDeals.map(deal => (
                                        <div key={deal.id} className="bg-white p-3.5 rounded-xl border border-slate-100 hover:shadow-md hover:border-blue-200/60 transition-all duration-200 cursor-pointer group">
                                            <div className="flex justify-between items-start mb-2">
                                                <span className="text-[10px] font-bold text-blue-700 bg-blue-50 ring-1 ring-blue-200/50 px-1.5 py-0.5 rounded">
                                                    {deal.probability}% Prob.
                                                </span>
                                                <button className="text-slate-400 hover:text-slate-600 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                                    <MoreHorizontal className="w-4 h-4" />
                                                </button>
                                            </div>
                                            <h4 className="font-semibold text-[13px] text-slate-800 mb-1">{deal.title}</h4>
                                            ...
                                            <div className="pt-2.5 border-t border-slate-100 flex justify-between items-center">
                                                ...
                                                {deal.technicalProposal?.validatedByEng && (
                                                    <Badge className="bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200/50 hover:bg-emerald-100 text-[9px] px-1.5 h-5 font-bold">
                                                        Eng. OK
                                                    </Badge>
                                                )}
                                            </div>
