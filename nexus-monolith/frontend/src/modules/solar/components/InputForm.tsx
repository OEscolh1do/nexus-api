...
export const InputForm: React.FC<Props> = ({ initialData, onSubmit }) => {
  ...
  // useCallback to make it stable for useEffect
  const updateLocation = React.useCallback((lat: number, lng: number, updateMap = false, shouldReverse = true) => {
  ...
  }, [formData.lat, formData.lng, updateLocation]); // Added dependencies
  ...
  const handleCaptureLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition((pos) => {
      ...
      // setGpsAccuracy(pos.coords.accuracy); // Removed unused setter
      setIsLocating(false);
    }, () => {
      setIsLocating(false);
        ...
        <section className="space-y-12">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-8 flex flex-col gap-6 relative">
              ...
              <div className="absolute top-6 left-1/2 -translate-x-1/2 z-50 flex gap-2 bg-neonorte-deepPurple/90 backdrop-blur-xl p-2.5 rounded-[1.8rem] border border-white/20 shadow-2xl">
              ...
              <div id={mapContainerId} className="w-full h-[550px] shadow-2xl rounded-[3.5rem] border-10 border-slate-50 overflow-hidden ring-1 ring-slate-200 cursor-crosshair relative z-10"></div>
