...
const Page: React.FC<{ children: React.ReactNode }> = ({ children }) => (
   <div className="proposal-page relative bg-white flex flex-col overflow-hidden text-slate-800 font-sans print:shadow-none print:m-0">
      ...
      <div className="grow p-[10mm] flex flex-col relative z-10">
         {children}
      </div>
   </div>
);
...
const Footer: React.FC<{ pageNum: number; totalPages: number }> = ({ pageNum, totalPages }) => (
   <footer className="mt-auto pt-4 border-t border-slate-100 flex justify-between items-center text-[8px] font-black text-slate-400 uppercase tracking-widest relative z-10">
      ...
      <div className="flex items-center gap-2">
         <span className="bg-slate-50 px-3 py-1 rounded-lg border border-slate-100">Ref: REF-{new Date().getFullYear().toString().slice(-2)}X99</span>
...
export const ProposalTemplate: React.FC<Props> = ({ data }) => {
   ...
   return (
      <div className="proposal-container bg-slate-200 print:bg-white min-h-screen font-sans">
         ...
         <Page>
            <Header />
            <main className="grow flex flex-col gap-5 relative z-10">
               ...
               <section className="grid grid-cols-3 gap-5">
                  {[
                     { icon: Activity, val: `${Number(data.systemSize).toFixed(2)}`, unit: "kWp", label: "Potência Instalada", color: "text-neonorte-purple", note: "Capacidade total de geração dos módulos." },
                     { icon: Zap, val: `${Math.round(data.monthlyAvgGeneration)}`, unit: "kWh", label: "Geração Média", color: "text-neonorte-green", note: "Energia média injetada na rede mensalmente." },
                     { icon: Target, val: `${data.resumo_financeiro.tempo_retorno_anos.toFixed(1)}`, unit: "ANOS", label: "Payback Estimado", color: "text-neonorte-deepPurple", note: "Tempo para recuperar 100% do capital investido." }
                  ].map((m, i) => (
                     <div key={i} className="bg-white p-6 rounded-[2.5rem] border border-slate-100 text-center shadow-lg border-b-4 flex flex-col items-center justify-center">
            ...
            <Page>
               ...
               <main className="grow flex flex-col gap-5">
                  ...
                  <div className="flex-1 bg-slate-900 rounded-[2.5rem] p-4 relative overflow-hidden shadow-2xl border border-slate-800 flex flex-col min-h-[400px]">
                     <div className="absolute top-0 left-0 w-full h-[60%] bg-linear-to-b from-black/50 to-transparent z-10 pointer-events-none"></div>

                     {/* Map Image Container */}
                     <div className="flex-1 rounded-[2rem] overflow-hidden relative border border-white/10 bg-slate-800">
                        {data.mapImage && <img src={data.mapImage} alt="Mapeamento do Telhado" className="w-full h-full object-cover" />}
                        ...
                        <div className="absolute top-6 left-6 z-20 bg-white/95 backdrop-blur-md px-6 py-4 rounded-2xl border border-site-100 shadow-xl">
                           ...
                           <p className="text-3xl font-black text-neonorte-deepPurple font-mono mt-2">{data.roofArea || 0} <span className="text-sm text-slate-400">m²</span></p>
                        </div>
                        ...
                        <div className="space-y-4">
                           ...
                           <div className="space-y-3 font-mono text-[10px]">
                              ...
                              <div className="flex justify-between items-end border-b border-slate-100 pb-1 border-dotted">
                                 ...
                                 <span className="font-black text-slate-700">{primaryModule.technology || 'PERC/TopCon'}</span>
                              </div>
                              <div className="flex justify-between items-end border-b border-slate-100 pb-1 border-dotted">
               ...
               <div className="bg-slate-50 rounded-[3rem] p-8 text-slate-800 relative overflow-hidden shadow-xl border border-slate-200">
                  <div className="grid grid-cols-2 gap-10 relative z-10 mb-8">
                     <div className="space-y-4">
                        ...
                        <p className="text-4xl font-black text-slate-800 font-mono italic tracking-tighter">{formatMoney(data.resumo_financeiro.total_hardware_estimado || 0)}</p>
                     ...
                     <div className="space-y-4 border-l border-slate-200 pl-8">
                        ...
                        <p className="text-4xl font-black text-neonorte-green font-mono italic tracking-tighter">{formatMoney(data.resumo_financeiro.total_servicos_contratados || 0)}</p>
               ...
               <div className="grid grid-cols-2 gap-6">
                  ...
                  <div className="space-y-3">
                     ...
                     <div className="bg-white rounded-[2.5rem] p-6 space-y-4 shadow-xl relative overflow-hidden border border-slate-100">
                        ...
                        <div className="absolute left-[2.2rem] top-8 bottom-8 w-px bg-slate-100 z-0"></div>
                     </div>
                  </div>
               </div>
