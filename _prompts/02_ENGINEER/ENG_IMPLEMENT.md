...
<execution_protocol>
  ...
  <step_1_backend>
    ...
    1. Ajuste `schema.prisma`.
    2. Rode a migração (ou comando equivalente).
    3. Atualize Services e Controllers conforme planejado.
    4. IMPORTANTE: Validação Zod em todos os inputs.
  </step_1_backend>

  <step_2_frontend>
    1. Atualize tipos/interfaces primeiro (Typescript).
    2. Implemente componentes visuais.
    3. Conecte ao backend.
    4. Trate estados de `Loading` (carregando) e `Error` (erro).
  </step_2_frontend>
  ...
  <step_4_documentation>
    Atualize o mapa do sistema: `{{CAMINHO_DA_DOC_PRINCIPAL}}`.
    Gere um `walkthrough.md` com o resumo do que foi feito.
  </step_4_documentation>
