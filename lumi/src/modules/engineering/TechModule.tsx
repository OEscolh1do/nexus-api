...
export const TechModule: React.FC = () => {
    ...
    const activeProjectId = useSolarStore(state => state.activeProjectId);

    // State to manage whether we are looking at the queue or a specific project
    const [isViewingQueue, setIsViewingQueue] = React.useState(!activeProjectId);

    // Sync isViewingQueue if activeProjectId changes externally
    React.useEffect(() => {
        setIsViewingQueue(!activeProjectId);
    }, [activeProjectId]);
    ...
    if (isViewingQueue) {
        return (
            <div className="h-[calc(100vh-64px)] w-full bg-slate-100 overflow-y-auto">
                <ProjectQueue onProjectLoaded={() => setIsViewingQueue(false)} />
            </div>
        );
    }
