...
import { useEquipmentStore } from '@/modules/crm/store/useEquipmentStore';
...
export const ModuleCatalogDialog: React.FC<ModuleCatalogDialogProps> = ({ 
    ...
    // Store - Catalog
    const { modules: availableModules, isLoading } = useEquipmentStore();
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
                ...
                <div className="relative z-50 w-full max-w-5xl bg-white rounded-xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200">
                    ...
                    <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between bg-slate-50/50 shrink-0">
                        <div>
                            <h2 className="text-lg font-bold text-slate-800">Catálogo de Módulos Fotovoltaicos</h2>
                            <p className="text-sm text-slate-500">Selecione os painéis para o arranjo.</p>
                        </div>
                        <button 
                            onClick={onClose}
                    ...
                    <div className="border-b border-slate-200 bg-white p-3 shrink-0 z-10 flex gap-3 items-center">
                         {/* Brand Select */}
                        ...
                        <div className="relative flex-1 max-w-sm">
                            ...
                            <input
                                ...
                                className="h-9 w-full rounded-md border border-slate-200 bg-transparent pl-9 pr-3 text-xs placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-400/20 focus:border-blue-400"
                            />
                        </div>

                         <div className="ml-auto text-xs text-slate-500">
                            <b>{filteredModules.length}</b> modelos encontrados
                        </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 bg-slate-50/30">
                        {isLoading && availableModules.length === 0 ? (
                            ...
                            <div className="h-full flex flex-col items-center justify-center p-12 text-center opacity-60">
                                ...
                                <h3 className="text-base font-semibold text-slate-600">Nenhum módulo encontrado</h3>
