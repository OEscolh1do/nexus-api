...
export const MapCanvasView: React.FC = () => {
  ...
  const isOverlayActive = focusedBlock !== 'arrangement' && focusedBlock !== 'map' && focusedBlock !== null;
  ...
  return (
    <div className="absolute inset-0 flex flex-col">
      ...
      <div className="relative flex-1 min-h-0">
        ...
        {!isOverlayActive && (
          <div className="absolute top-3 left-3 lg:top-6 lg:left-6 z-[1000] flex flex-col gap-1 lg:gap-1.5 p-1 lg:p-1.5 rounded-lg bg-slate-900/90 border border-slate-800 backdrop-blur-md shadow-2xl transition-all animate-in fade-in duration-300">
            {visibleTools.map((tool) => (
              <button
                ...
                className={cn(
                  "p-1.5 lg:p-2 rounded-md transition-all flex items-center justify-center",
                  activeTool === tool.id
                    ? "bg-emerald-500 text-slate-900 shadow-lg scale-105"
        ...
        {selectedEntity.type === 'string' && (
          <div className="absolute bottom-12 lg:bottom-16 left-1/2 -translate-x-1/2 w-[calc(100vw-32px)] lg:w-[340px] z-[1000] animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      {!isOverlayActive && <MapContextBar mode={mapMode} />}
    </div>
  );
};
