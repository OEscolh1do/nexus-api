...
const VisibilityBadge: React.FC<{
  ...
  <button
    ...
    className={cn(
      "flex items-center gap-1.5 px-2.5 py-1.5 sm:px-2.5 sm:py-2 rounded-sm border text-[10px] sm:text-[10px] font-black uppercase tracking-wider transition-all duration-200 shrink-0",
      active
...
export const ProposalEditPanel: React.FC = () => {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      {/* ── Header & Navigation ─────────────────────────────────────── */}
      <div className="p-4 border-b border-slate-800 bg-slate-900/40 flex flex-col gap-2 sm:gap-3 shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-3">
            <FileText size={14} className="text-indigo-400" />
            <span className="text-[10px] sm:text-xs font-black uppercase tracking-[0.2em] text-slate-300">
              Proposta
            </span>
          </div>
          <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-hide pb-1 -mr-4 pr-4">
            <button 
              onClick={() => updateProposalData({ excludedPages: [] })}
              className="text-[10px] sm:text-[11px] font-black uppercase tracking-wider px-2 py-1.5 sm:px-3 sm:py-2 bg-slate-800/50 rounded-sm text-slate-400 hover:text-white hover:bg-slate-700 transition-colors whitespace-nowrap"
          ...
          >
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              {proposalData.excludedPages?.includes(activePage) && (
                <span className="text-xs font-bold text-amber-500 ml-2 border border-amber-500/30 px-1.5 rounded-sm bg-amber-500/10">
                  Oculto
                </span>
              )}
            </div>
                ...
                return (
                  <div key={label} className="flex items-center group px-1">
                    <button
                      ...
                      className={cn(
                        "flex-1 flex items-center gap-2 sm:gap-3 p-3 rounded-sm text-left transition-colors",
      ...
      <div className="flex-1 overflow-y-auto p-3 sm:p-4 flex flex-col gap-2 sm:gap-3 scrollbar-hide sm:custom-scrollbar">
        ...
        {activePage === 0 && (
          <div className="flex flex-col gap-2 sm:gap-3">
            <label className="text-[10px] sm:text-[11px] font-black uppercase tracking-widest text-slate-600">
              Dados da Capa
            </label>
          ...
          <>
            ...
            <div className="flex flex-col gap-2 sm:gap-3">
                ...
                <div key={item.id} className="flex items-start gap-2 sm:gap-3 p-3 sm:p-3 bg-slate-900/60 border border-slate-800 rounded-sm">
                  ...
                  <div className="flex-1 flex flex-col gap-1.5">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                      <input
                        type="number"
                        className="w-full sm:w-24 bg-slate-950 border border-slate-800 rounded-sm px-2 py-1.5 sm:px-3 sm:py-2 text-[11px] sm:text-xs text-white font-mono outline-none focus:border-indigo-500/50"
                        placeholder="R$ 0,00"
                        value={item.value ?? ''}
                        onChange={e => {
              ...
              {(proposalData.lineItems || []).length > 0 && (
                <div className="flex justify-end px-2">
                  <span className="text-[10px] sm:text-xs font-black text-indigo-400 tabular-nums">
            ...
            <div className="flex flex-col gap-2 sm:gap-3">
              ...
              {(proposalData.paymentStages || []).map((stage) => (
                <div key={stage.id} className="flex items-center gap-2 sm:gap-3 p-3 sm:p-3 bg-slate-900/60 border border-slate-800 rounded-sm flex-wrap sm:flex-nowrap">
                  <input
                  ...
                  <input
                    type="number"
                    className="w-20 bg-slate-950 border border-slate-800 rounded-sm px-2 py-1.5 sm:px-3 sm:py-2 text-[11px] sm:text-xs text-white font-mono outline-none focus:border-indigo-500/50"
                    placeholder="R$"
                    value={stage.value || ''}
            ...
            <div className="flex flex-col gap-2 sm:gap-3">
              ...
              <textarea
                className="w-full bg-slate-950 border border-slate-800 rounded-sm p-3 sm:p-3 text-xs sm:text-sm text-slate-300 font-medium outline-none focus:border-indigo-500/50 min-h-[60px] resize-none"
        ...
        {activePage === 2 && (
          <div className="flex flex-col gap-2 sm:gap-3">
            ...
            <textarea
              className="w-full bg-slate-950 border border-slate-800 rounded-sm p-3 sm:p-3 text-xs sm:text-sm text-slate-300 font-medium outline-none focus:border-indigo-500/50 min-h-[80px] resize-none"
        ...
        {activePage === 3 && (
          <div className="flex flex-col gap-2 sm:gap-3">
            ...
            {(proposalData.executionSchedule || []).map((stage) => (
              <div key={stage.id} className="flex flex-col gap-1.5 p-3 sm:p-3 bg-slate-900/60 border border-slate-800 rounded-sm">
                <div className="flex items-center gap-2 sm:gap-3">
        ...
        {activePage === 4 && (
          <div className="flex flex-col gap-2 sm:gap-3">
            ...
            <div className="flex flex-col gap-1.5">
              <span className="text-[9px] sm:text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1">
            ...
            <div className="grid grid-cols-2 gap-2 sm:gap-3">
              ...
              <div className="flex flex-col gap-1.5">
                <span className="text-[9px] sm:text-xs font-bold text-slate-500 uppercase tracking-wider">CREA</span>
                <input
                  className="w-full bg-slate-950 border border-slate-800 rounded-sm px-2 py-1.5 sm:px-3 sm:py-2 text-[11px] sm:text-xs text-white font-mono outline-none focus:border-indigo-500/50"
      ...
      <div className="p-4 border-t border-slate-800 bg-[#0a0f1a] flex flex-col gap-2 sm:gap-3">
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-hide -mx-4 px-4 pb-1">
          <span className="text-[10px] sm:text-xs font-black text-slate-600 uppercase tracking-widest mr-1 shrink-0">PDF</span>
          <VisibilityBadge
            label="Preços"
        ...
        <button
          ...
          className={cn(
            "w-full h-9 sm:h-10 bg-indigo-600 hover:bg-indigo-500 text-white rounded-sm flex items-center justify-center gap-2 sm:gap-3 transition-all active:scale-[0.98] shadow-lg shadow-indigo-950/40 border border-indigo-400/20 group",
