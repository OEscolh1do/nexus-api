...
import { useModules, type ModuleEquipment } from '@/hooks/useCatalog';
...
export default function ModulesDataGrid({ refreshTrigger }: { refreshTrigger: number }) {
  const [page, setPage] = useState(1);
  const [rawQ, setRawQ] = useState('');
  const [filters, setFilters] = useState<Omit<Filters, 'q'>>({ isActive: '' });
  const [selectedModule, setSelectedModule] = useState<ModuleEquipment | null>(null);
  ...
  return (
    <div className="flex h-full flex-col gap-4 mt-4">
      ...
      <div className="flex flex-1 flex-col overflow-hidden rounded-sm border border-slate-800 bg-slate-900">
          ...
          <table className="w-full min-w-[800px] border-collapse text-left">
            <thead className="sticky top-0 z-10 bg-slate-900">
              <tr className="border-b border-slate-800">
                ...
                <th className="px-4 py-2.5 text-[11px] font-medium uppercase tracking-wider text-slate-500">Potência</th>
            ...
            <tbody>
              ...
              {!loading && modules.map((m) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-xs text-slate-300">{m.powerWp} Wp</td>
        ...
        {pagination && (
          <div className="flex items-center justify-between border-t border-slate-800 px-4 py-3">
            <p className="text-[11px] text-slate-500">{pagination.total} módulos</p>
      ...
      {selectedModule && (
        <ModuleDrawer
          moduleEquipment={selectedModule}
