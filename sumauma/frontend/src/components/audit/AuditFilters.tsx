...
export default function AuditFilters({ filters, setFilters, onClear }: AuditFiltersProps) {
  ...
  return (
    <div className="flex flex-wrap items-end gap-3 p-4 bg-slate-900 border border-slate-800 rounded-sm">
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-[10px] font-medium uppercase text-slate-500">Resource ID</label>
        <input
          name="resourceId"
          value={filters.resourceId || ''}
          onChange={handleChange}
          placeholder="Ex: proj_123"
          className="h-8 w-32 px-3 text-xs bg-slate-800 border border-slate-700 rounded-sm text-slate-200 focus:outline-none focus:border-sky-500/50"
        />
      </div>
